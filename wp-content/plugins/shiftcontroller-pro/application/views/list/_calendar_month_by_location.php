...
if( $can_add ){
	if( ! $acl->set_object( $test_shift->set('type', $test_shift->_const('TYPE_SHIFT')) )->can('add') ){
		$can_add = FALSE;
	}
}
...
foreach( $locations as $location ){
	...
	$quickform_view = NULL;
	if( $form ){
		$quickform_view = HC_Html_Factory::widget('module')
			->set_url( $rootlink . '/quickform' )
			->pass_arg( array($entity_shifts, $this_state) )
			->set_params( $this_state )
			->set_param( 'location', $entity_id )
			->set_show_empty( TRUE )
	...
	$QUICKFORM[$entity_id] = $quickform_view;
...
foreach( $locations as $location ){
	...
	if( $current_user_id ){
		...
		if( isset($QUICKFORM[$entity_id]) ){
			$title->add_item('quickform', $QUICKFORM[$entity_id] );
			$title->add_item_attr('quickform', 'class', 'text-small' );
		}
	}
	else {
